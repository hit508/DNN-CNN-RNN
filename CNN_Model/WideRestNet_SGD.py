...
WEIGHT_DECAY = 0.0005
ROOT_PATH = 'drive/MyDrive/Model/WideResNet'
LOG_FILE_PATH = os.path.join(ROOT_PATH, 'logs')
MODEL_FILE_PATH = os.path.join(ROOT_PATH, 'model')
...
if __name__ == '__main__':
    ...
    # set optimizer
    sgd = keras.optimizers.SGD(learning_rate=.1, momentum=0.9, nesterov=True)
    ...
    # start training
    resnet.fit(datagen.flow(x_train, y_train, batch_size=BATCH_SIZE),
               steps_per_epoch=ITERATIONS,
               epochs=EPOCHS,
               callbacks=cbks,
               validation_data=(x_test, y_test))
    resnet.save(MODEL_FILE_PATH)
