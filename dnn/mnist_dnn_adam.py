...
s = tf.Variable(tf.zeros(shape=[784 * 500 + 500 * 10 + 500 + 10]), trainable=False)
...
for k in range(epoch):
    ...
    for train_step in range(batch_num):
        ...
        # ADAM自适应学习率
        t += 1
        s = adam_param1 * s + (1 - adam_param1) * grad
        r = adam_param2 * r + (1 - adam_param2) * tf.multiply(grad, grad)

        s_predict = s / (1 - tf.math.pow(adam_param1, t))
        r_predict = r / (1 - tf.math.pow(adam_param2, t))

        deta_theta = -learning_rate * (s_predict / (tf.math.sqrt(r_predict) + adam_deta))
        train_variable = train_variable + deta_theta
        tf.nn.dropout(train_variable, rate=0.5, seed=1)

    image_train = tf.cast(tf.reshape(x_train, shape=[60000, 784]), tf.float32)
    y_train_predict = tf.nn.softmax(forward_prop(image_train, train_variable), axis=1)
...
y_test_predict = tf.nn.softmax(forward_prop(image, train_variable), axis=1)
